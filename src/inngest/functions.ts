...
export const processVideo = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        const {userId,credits,s3Key}=await step.run("check-credits", async () => {
            ...
            return {
                userId: uploadedFile.user.id, credits: uploadedFile.user.credits,s3Key:uploadedFile.s3Key
        ...
        const {clipsFound}=await step.run("create-clips-in-db",async()=>{
                ...
                const clipKeys=allKeys.filter((key: string | undefined):key is string => key!== undefined && !key.endsWith("original.mp4") )
